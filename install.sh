...
detect_binary() {
    ...
    case "$os" in
        linux)
            BINARY_NAME="coraza-waf-proxy-linux-${arch}"
            ;;
        mingw*|cygwin*|msys*)
            os="windows"
            BINARY_NAME="coraza-waf-proxy-windows-${arch}.exe"
            ;;
        darwin)
            os="macos"
            BINARY_NAME="coraza-waf-proxy-macos-${arch}"
            ;;
        *)
            echo "错误：不支持的操作系统：$os"
            exit 1
...
[Unit]
Description=Coraza WAF Proxy 服务
...
show_usage() {
    echo "用法：$0 {install|uninstall|enable|disable|start|stop|restart|status}"
    echo ""
    echo "命令："
...
main() {
    if [ $# -eq 0 ]; then
        echo "Coraza WAF Proxy 安装脚本"
        echo "====================================="
        echo ""
        echo "此脚本将帮助您安装/卸载 coraza-waf-proxy 服务"
        echo ""
